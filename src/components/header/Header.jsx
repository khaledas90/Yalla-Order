...
export default function Header({ MainPage, IconOne, IconTwo, IconThree, IconFour }) {
  ...
  function handleOpenBag() {
    setOpenMenu(openMenu === "bag" ? "" : "bag");
  }
  function handleOpenFav() {
    setOpenMenu(openMenu === "fav" ? "" : "fav");
  }
  function handleOpenLang() {
    setOpenMenu(openMenu === "lang" ? "" : "lang");
  }
  function handleOpenProf() {
    setOpenMenu(openMenu === "prof" ? "" : "prof");
  }
          ...
          {navStatus ? (
            <>
              ...
              >
                <Nav>
                  {token ? <>
                    <Nav.Link to="/" className={`links text-white d-flex justify-content-center`}>
                      <Link exact="true" to="/"> Home</Link>
                    </Nav.Link>
                    <Nav.Link to="/clinics" className={`links text-white d-flex justify-content-center`}>
                      <Link to={MainPage === "Restaurants" ? "/Restaurants" : "/CLinics"}>{MainPage}</Link>
                    </Nav.Link>
                    <Nav.Link className={`links text-white d-flex justify-content-center`}>
                ...
                {loginBtn && !isLoggedIn && (
                  <div className="d-flex justify-content-center">
                    <Link exact="true" className="LinkLogin" to="/Login">
                      <button className={`btnStyle btn text-white text-center mt-3 active`}>
                        Login
                      </button>
                    </Link>
                  </div>
            ...
            <>
              ...
              <div className="d-flex gap-2 align-items-center">
                <div className="navBarIcons d-flex gap-3 align-items-center">
                  ...
                  {!isLoggedIn && (
                    <Link exact="true" className="LinkLogin" to="/Login">
                      <button className={`btnStyle btn text-white ms-2 text-center`}>
                        Login
                      </button>
                    </Link>
                  )}
