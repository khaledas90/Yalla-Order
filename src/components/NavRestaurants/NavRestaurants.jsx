...
function NavRestaurants() {
  ...
  return (
    <div className="navBar">
      <div className="logo"><Link to="/"><img src={logoImg} alt='insta order'/></Link></div>
      <ul className={`nav-links ${menuOpen ? 'active' : ''}`}>
        <li><NavLink to="/HomeRestaurants">Home</NavLink></li>
        <li><NavLink to="/restaurants">Restaurants</NavLink></li>
        <li><NavLink to="/BecomeAPartner">Become a Partner</NavLink></li>
        <li><NavLink to="/trackOrders">Track Orders</NavLink></li>
        <li><NavLink to="/AboutUs">About Us</NavLink></li>
      </ul>
      <div className="icons">
        {['favorite', 'bag', 'language', 'profile'].map((icon) => (
          <div className="icon" key={icon} onClick={() => toggleDropdown(icon)}>
            {icon === 'favorite' && <div className='iconContainer' onClick={handleFetchFavorites}><FavoriteBorderOutlinedIcon/>{favCount !== 0 &&  <span className='Count'>{favCount}</span>}</div> }
            {icon === 'bag' && <div className='iconContainer' ><ShoppingBagOutlinedIcon/> {bagCount !== 0 &&  <span className='Count'>{bagCount}</span>}</div>}
            {icon === 'language' && <LanguageOutlinedIcon/>}
            {icon === 'profile' && <AccountCircleOutlinedIcon/>}
            <div className={`dropdown ${dropdownOpen[icon] ? 'show' : ''}`}>
             {icon === 'favorite' && <FavRestaurant favorites={favorites} loadingFavorites={loadingFavorites} favoritesError={favoritesError}/>}
             {icon === "bag" && <Bag bagItems={bagItems} setBagItems={setBagItems} loadingBagItems={loadingBagItems} setLoadingBagItems={setLoadingBagItems} bagItemsError={bagItemsError} setBagItemsError={setBagItemsError}/>}
